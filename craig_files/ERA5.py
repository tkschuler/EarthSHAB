"""
This version was tailored to work with the flight_simulater.py
and trapezoid_era.py. program.
...
import properties.era5_properties as netcdf_prop
...
class ERA5:
    def __init__(self, start_coord, end_coord, input_file, dt_sec, sim_time_hours, use_time):
        ...
        input_file : str
            file name of NetCDF ERA5 climate data. Its name is relative to
            where the code is run from.
        dt_sec : float
            Sampling rate of simulation in seconds
        sim_time_hours : integer or None
            Number of hours to do simulation, can be None, then will try to
            get simulation time off of start_coord - end_coord
        use_time: boolean
            When false don't extract time variable from the NETCDF data
        ...
        self.resolution_hr = netcdf_prop.netcdf["resolution_hr"]
        self.num_model_hrs = sim_time_hours
        if use_time:
    ...
    def init_with_time(self, start_coord, end_coord, dt_sec, sim_time_hours):
        ''' adds to class object containing information about an ERA5 but without any time
        variables.
        ...
        model_hours = (mt.datetime_epoch(self.model_end_datetime) - mt.datetime_epoch(self.model_start_datetime))/3600
        ...
        if self.num_model_hrs == None:
            self.simulation_points = int((mt.datetime_epoch(self.model_end_datetime) - mt.datetime_epoch(self.model_start_datetime))/dt_sec)
    ...
    def getNewCoord(self, coord, dt):
        ...
        z = self.getNearestAltbyIndex(int_hr_idx, lat_idx, lon_idx, coord["alt_m"])  # fix this for lower and Upper

        try:
            x_wind_vel, y_wind_vel = self.wind_alt_Interpolate(coord['alt_m'], diff_time, lat_idx, lon_idx)  # for now hour index is 0
            if x_wind_vel == None:
